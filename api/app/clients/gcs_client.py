...
def get_run_report_uri(run_id: str, bucket_name: str | None = None) -> Dict[str, str]:
    ...
    report_path = metadata["artifacts"].get("report_html_path")

    if not report_path:
        raise FileNotFoundError(f"report path missing for run_id={run_id}")

    blob_path = f"runs/{run_id}/{report_path}"
    return {
        "run_id": run_id,
        "bucket": resolved_bucket,
        "report_path": report_path,
        "report_uri": build_gcs_uri(resolved_bucket, blob_path),
    }
...
def get_run_qc_uri(run_id: str, bucket_name: str | None = None) -> Dict[str, str]:
    ...
    if stdout_log_path:
        ...
        result["stdout_log_uri"] = build_gcs_uri(resolved_bucket, f"runs/{run_id}/{stdout_log_path}")

    if stderr_log_path:
        result["stderr_log_path"] = stderr_log_path
        result["stderr_log_uri"] = build_gcs_uri(resolved_bucket, f"runs/{run_id}/{stderr_log_path}")
